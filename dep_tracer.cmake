function(validate_shell_script_extension FILE)
    ...
    set(SHARED_EXTENSIONS "py|cmake\\.json|custom\\.json|meson\\.json|file\\.json")
...
function(set_shell_program SCRIPT_FILENAME PYTHON_NAME_SUFFIX)
    ...
    if(SCRIPT_EXT STREQUAL "filejson")
		set(SCRIPT_RUNNER "FileFetcher")
		set(SCRIPT_ARG "")
	endif()
...
macro(trace_dependency)
    set(oneValueArgs "INSTALL_SCRIPT" "NAME" "VERSION" "COMPONENT_INFIX" "PYTHON_NAME_SUFFIX" "WORKING_DIR" "CUSTOM_SEARCH_DIR")
    ...
    set(MOD_SEARCH_DIR OFF)
    set(PACKAGE_SEARCH_DIR ${CMAKE_INSTALL_PREFIX})
    if(DEFINED PACKAGE_CONTROLLER_CUSTOM_SEARCH_DIR)
        set(PACKAGE_SEARCH_DIR ${PACKAGE_CONTROLLER_CUSTOM_SEARCH_DIR})
        set(MOD_SEARCH_DIR ON)
    endif()

    if(TREATED_LIMIT_SEARCH_PATHS)
        if(USE_EXTRA_SUFFIX)
            find_package(${PACKAGE_CONTROLLER_NAME} ${VERSION_ARG} ${PACKAGE_CONTROLLER_VERSION} COMPONENTS ${PACKAGE_CONTROLLER_COMPONENTS} PATHS ${PACKAGE_SEARCH_DIR} NO_DEFAULT_PATH PATH_SUFFIXES ${PACKAGE_CONTROLLER_EXTRA_SUFFIXES})
        else()
            find_package(${PACKAGE_CONTROLLER_NAME} ${VERSION_ARG} ${PACKAGE_CONTROLLER_VERSION} COMPONENTS ${PACKAGE_CONTROLLER_COMPONENTS} PATHS ${PACKAGE_SEARCH_DIR} NO_DEFAULT_PATH)
        endif()
    else()
    ...
    if(TREATED_LIMIT_SEARCH_PATHS)
        find_package(${PACKAGE_CONTROLLER_NAME} ${VERSION_ARG} ${PACKAGE_CONTROLLER_VERSION} COMPONENTS ${PACKAGE_CONTROLLER_COMPONENTS} PATHS ${PACKAGE_SEARCH_DIR} NO_DEFAULT_PATH REQUIRED ${EXTRA_SUFFIX_ARG} ${TREATED_EXTRA_SUFFIXES})
    else()
...
macro(trace_library)
    set(oneValueArgs "INSTALL_SCRIPT" "NAME" "PYTHON_NAME_SUFFIX" "WORKING_DIR" "CUSTOM_SEARCH_DIR")
    ...
    if(NOT DEFINED LIBRARY_CONTROLLER_PYTHON_NAME_SUFFIX)
        set(LIBRARY_CONTROLLER_PYTHON_NAME_SUFFIX "<EMPTY>")
    endif()
    ...
    set(MOD_SEARCH_DIR OFF)
    set(LIBRARY_SEARCH_DIR ${CMAKE_INSTALL_PREFIX}/lib)
    if(DEFINED LIBRARY_CONTROLLER_CUSTOM_SEARCH_DIR)
        set(LIBRARY_SEARCH_DIR ${LIBRARY_CONTROLLER_CUSTOM_SEARCH_DIR})
        set(MOD_SEARCH_DIR ON)
    endif()

    if(TREATED_LIMIT_SEARCH_PATHS)
        find_library(${LIBRARY_CONTROLLER_NAME}_FOUND NAMES ${LIBRARY_CONTROLLER_NAME} PATHS ${LIBRARY_SEARCH_DIR} NO_DEFAULT_PATH)
    else()
        find_library(${LIBRARY_CONTROLLER_NAME}_FOUND NAMES ${LIBRARY_CONTROLLER_NAME} PATHS ${LIBRARY_SEARCH_DIR})
    endif()
    ...
    set(MOD_SEARCH_DIR OFF)
    set(LIBRARY_SEARCH_DIR ${CMAKE_INSTALL_PREFIX})
    if(DEFINED LIBRARY_CONTROLLER_CUSTOM_SEARCH_DIR)
        set(LIBRARY_SEARCH_DIR ${LIBRARY_CONTROLLER_CUSTOM_SEARCH_DIR})
        set(MOD_SEARCH_DIR ON)
    endif()
    ...
    if(TREATED_LIMIT_SEARCH_PATHS)
        find_library(${LIBRARY_CONTROLLER_NAME}_FOUND NAMES ${LIBRARY_CONTROLLER_NAME} PATHS ${LIBRARY_SEARCH_DIR} NO_DEFAULT_PATH REQUIRED)
    else()
        find_library(${LIBRARY_CONTROLLER_NAME}_FOUND NAMES ${LIBRARY_CONTROLLER_NAME} PATHS ${LIBRARY_SEARCH_DIR} REQUIRED)
    endif()
...
macro(trace_file)
    ...
    if(NOT DEFINED FILE_CONTROLLER_PYTHON_NAME_SUFFIX)
        set(FILE_CONTROLLER_PYTHON_NAME_SUFFIX "<EMPTY>")
    endif()
